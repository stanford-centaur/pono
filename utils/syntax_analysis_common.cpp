...
std::string convert_bin_str_to_decimal(const std::string & in)
{
  ...
  for (auto pos = in.begin(); pos != in.end(); ++pos) {
    ...
    if (*pos == '1') syntax_analysis::add1(out);
  }
  std::string ret;
...
{
  // both str
  if (sv.length() < r.sv.length()) return true;
  if (sv.length() > r.sv.length()) return false;
  for (size_t pos = 0; pos < sv.length(); ++pos) {
    // if (sv.at(pos) == '0' && r.sv.at(pos) == '1')
    if (sv.at(pos) < r.sv.at(pos)) return true;
    // if (sv.at(pos) == '1' && r.sv.at(pos) == '0')
    if (sv.at(pos) > r.sv.at(pos)) return false;
  }
...
smt::Term smt_string_to_const_term(const std::string & val, smt::SmtSolver & s)
{
  if (val == "true" || val == "True") {
    ...
    uint64_t width =
        (uint64_t)(StrToULongLong(ReplaceAll(v_w_pair[1], ")", ""), 10));
    auto bvsort = s->make_sort(smt::SortKind::BV, width);
...
{
  return ((a.input_width == b.input_width) && (a.op == b.op)
          && (a.extw == b.extw));
}
...
void SyntaxStructure::insert_symbol(uint64_t width, const std::string & name)
{
  ...
  if (res.second) new_constructs = true;  // indeed inserted
}
...
void SyntaxStructure::AndOrConvert()
{
  for (auto & width_cnstr : syntax_) {
    ...
    if ((IN(smt::PrimOp::BVAnd, cnstr.op_binary)
        ...
        && (IN(smt::PrimOp::BVOr, cnstr.op_binary)
            || IN(smt::PrimOp::Or, cnstr.op_binary))
        && (IN(smt::PrimOp::BVNot, cnstr.op_unary)
            || IN(smt::PrimOp::Not, cnstr.op_unary))) {
...
void SyntaxStructure::RemoveUnusedStructure()
{
  ...
  for (auto && width_cnstr : syntax_) {
    ...
    if (!cnstr.constants.empty() || !cnstr.symbol_names.empty()) {
      q.push(width);
      use_map[width].insert(width);
    }
    if (!cnstr.op_unary.empty() || !cnstr.op_binary.empty()
        || !cnstr.op_concat.empty())
      use_map[width].insert(width);
    ...
    if (IN(pos->first, reachable_set)) {
      ...
      {  // concat
        ...
        while (concat_pos != pos->second.op_concat.end()) {
          if (!IN(concat_pos->width1, reachable_set)
              || !IN(concat_pos->width2, reachable_set))
