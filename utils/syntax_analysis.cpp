...
unsigned VarTermManager::GetMoreTerms(IC3FormulaModel * pre,
                                      IC3FormulaModel * post,
                                      TermLearner & term_learner,
                                      ...
                                      SyGuSTermMode term_mode)
{
  ...
  switch (varset_info.state.stage) {
    ...
    case PerVarsetInfo::state_t::WPARTIAL:
      // increase to WALL
      {
        unsigned nterm_walked =
            insert_from_termsmap_w_width(varset_info.terms_buffer /*IN*/,
                                         varset_info /*OUT*/,
                                         varset_info.state.partial_width_done,
                                         (unsigned)(-1));
    ...
    case PerVarsetInfo::state_t::FROMCEX:
      ...
      {
        auto nterms = failed_at_init
                          ? term_learner.learn_terms_from_cex_same_frame(
                                pre, post, trans, /*output*/ varset_info)
                          : term_learner.learn_terms_from_cex(
                                pre, post, trans, /*output*/ varset_info);
...
const PerVarsetInfo & VarTermManager::SetupTermsForVarModelNormal(
    ...
    unsigned term_extract_depth,
    unsigned initial_term_width,
    unsigned initial_term_inc,
    unsigned accumulated_term_bound)
{
  bool collect_constant = width_to_constants_.empty();
  std::unordered_set<smt::Term> varset;
  m->get_varset(varset);

  terms_cache_t::iterator pos;
  bool succ;
  std::tie(pos, succ) = terms_cache_.emplace(
      canonical_string, PerVarsetInfo(PerVarsetInfo::state_t::EMPTY));

  // now TERM_EXTRACT_DEPTH
  TermExtractor extractor(varset,
                          collect_constant,
                          term_extract_depth,
                          pos->second.terms_buffer,
                          pos->second.all_terms);

  for (auto && t : terms_to_check_) extractor.WalkBFS(t);

  if (collect_constant) insert_from_constmap(extractor.GetConstants());
  ...
  do {
    nterm_walked += insert_from_termsmap_w_width(
        terms /*IN*/, term_cache_item /*OUT*/, width_start, width_end);
    ...
    smt::SmtSolver & solver_)
{
  ...
  bool succ;
  std::tie(pos, succ) = terms_cache_.emplace(
      canonical_string, PerVarsetInfo(PerVarsetInfo::state_t::EXTRACTBITS));
...
void VarTermManager::insert_split(PerVarsetInfo & term_cache_item /*OUT*/,
                                  ...
                                  smt::SmtSolver & solver_)
{
  ...
  for (const auto & sv_slice_pair : sv_slices) {
    ...
    for (const auto & lr_pair : slices) {
      auto t = solver_->make_term(
          smt::Op(smt::PrimOp::Extract, lr_pair.first, lr_pair.second), sv);
      ...
      if (res.second) term_cache_item.terms[w].terms.push_back(t);
      // Parent relation? All_terms ?
    }
  }
}  // insert_split

void VarTermManager::insert_vars_only(PerVarsetInfo & term_cache_item /*OUT*/,
                                      const smt::UnorderedTermSet & varset)
{
  for (const auto & v : varset) {
    ...
    if (res.second) term_cache_item.terms[width].terms.push_back(v);
  }
}  // insert_vars_only
...
void VarTermManager::term_const_w1_const(
    PerVarsetInfo & term_cache_item /*OUT*/, smt::SmtSolver & solver_)
{
...
void VarTermManager::const_to_per_varset(
    PerVarsetInfo & term_cache_item /*OUT*/,
    ...
    unsigned & nterm_walked)
{
  ...
  for (auto && w_cs_pair : width_to_constants_) {
    ...
    if (!(width >= width_bound_low && width < width_bound_high)) continue;
    ...
    unsigned width_bound_high /*IN*/)
{
  ...
  const_to_per_varset(
      term_cache_item, width_bound_low, width_bound_high, nterm_walked);
  return nterm_walked;
}  // insert_from_termsmap_w_width

void VarTermManager::insert_from_constmap(
    const PerVarsetInfo::width_term_map_t & w_c_map)
{
