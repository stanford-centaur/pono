...
void Walker::WalkDFS(const smt::Term & ast)
{
  if (Skip(ast)) return;
  PreChild(ast);
  for (const auto & c : *ast) WalkDFS(c);
  PostChild(ast);
}
  ...
  if (ast->is_symbolic_const()) {
    ...
    switch (op.prim_op) {
      ...
      case smt::PrimOp::BVNot:
        constructs.insert_op_unary(width, op.prim_op);
        break;
      case smt::PrimOp::And:
      case smt::PrimOp::Or:
      case smt::PrimOp::Xor:
      ...
      case smt::PrimOp::BVLshr:
        constructs.insert_op_binary(width, op.prim_op);
        break;
      case smt::PrimOp::BVComp:  // equal
        constructs.insert_op_comp(width, smt::PrimOp::Equal);
        break;
...
void TermExtractor::PostChild(const smt::Term & ast)
{
  ...
  if (ast->is_symbolic_const()) {
    ...
    if (level_ > 0)
      ++max_level;  // if given level_ > 0, then we will count lv, otherwise, we
                    // don't care
...
bool TermScore::Skip(const smt::Term & ast) { return IN(ast, scores_); }
...
void ConstantExtractor::PostChild(const smt::Term & ast)
{
  ...
  if (ret.second) width_constant_map[width].push_back(ast);
}  // PostChild
...
void SliceExtractor::PostChild(const smt::Term & ast)
{
  ...
  if (ast->is_value()) return;
  if (ast->is_symbolic_const()) return;
  ...
  if (op.prim_op == smt::PrimOp::Extract) {
    ...
    for (const auto & sv : varset) {
      if (IN(sv, related_vars_)) {
        ...
        if (l < w) sv2exts_[sv].insert(std::make_pair(l, r));
...
unsigned TermLearner::vars_extract_bit_level(
    IC3FormulaModel * post, /*OUTPUT*/ PerVarsetInfo & varset_info)
{
  ...
  for (const auto & v : vars) {
    ...
    for (unsigned idx = 0; idx < width; ++idx) {
      ...
      // std::cout << "Extract " << v->to_string() << "[" << idx << "] is_new:"
      //           << is_new << std::endl;
      nterm += is_new ? 1 : 0;
    }
  }
...
unsigned TermLearner::learn_terms_from_cex(
    IC3FormulaModel * pre_full_model,
...
unsigned TermLearner::same_val_replace_ast(
    /*INOUT*/ PerVarsetInfo & varset_info)
{
  unsigned n_new_terms = 0;
  for (auto & width_info_pair : varset_info.terms) {
    auto width = width_info_pair.first;
    if (width <= 1) continue;
    std::unordered_map<eval_val, old_new_terms, eval_val_hash>
        eq_class;  // equivalent classes
...
#if REPLACE_DEBUG_LVL >= 1
      std::cout << "EQ class, val: " << val_tvec_pair.first.to_string() << " w"
                << width << " #old:" << tvec_old.size()
                << " |-> #new:" << tvec_new.size() << "\n  * ";
      for (const auto & t : tvec_old) std::cout << t->to_string() << " , ";
      std::cout << " :|old -> new|: ";
      for (const auto & t : tvec_new) std::cout << t->to_string() << " , ";
      std::cout << std::endl;
#endif
      // old -> new & new -> new
...
unsigned TermLearner::same_val_replace_ast_same_frame(
    /*INOUT*/ PerVarsetInfo & varset_info)
{
  unsigned n_new_terms = 0;
  for (auto & width_info_pair : varset_info.terms) {
    ...
    for (const auto & val_tvec_pair : eq_class) {
      ...
      std::cout << "EQ class, val: " << val_tvec_pair.first.to_string() << " w"
                << width << " #old:" << tvec_old.size()
                << " | replaced internally \n  * ";
      for (const auto & t : tvec_old) std::cout << t->to_string() << " |end| ";
      std::cout << std::endl;
#endif
...
unsigned TermLearner::replace_hierachically(const smt::Term & orig,
                                            const smt::Term & repl,
                                            PerVarsetInfo & varset_info)
{
  if (orig->to_string() == repl->to_string()) return 0;
    ...
    smt::TermVec & output_new_terms)
{
  ...
  if (parentvec.empty()) return 0;  // if has no parent term, no replacement
  unsigned nterm = 0;
  ...
  for (const auto & p : parentvec) {
    ...
    smt::TermVec new_terms;
    {
      ...
      for (auto idx : child_pos) {
        ...
        if (is_new_term) {
          ...
          RD2(3,
              "  [ReplaceParent]    in parent (exists): {} ==> {}",
              p->to_string(),
              new_parent->to_string());
        }
    ...
    for (const auto & nt : new_terms) {
      RD2(1,
          ...
          nt->to_string());
      nterm +=
          replace_hierachically_w_parent(p, nt, varset_info, output_new_terms);
