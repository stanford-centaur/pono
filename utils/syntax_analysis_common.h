...
struct eval_val
{
  ...
  bool operator==(const eval_val & r) const { return (sv == r.sv); }
...
struct PerCexInfo
{
  ...
  {
    predicates_nxt.clear();
    predicates_str.clear();
  }
};
...
// and not reliable
typedef std::unordered_map<IC3FormulaModel *, PerCexInfo>
    cex_term_map_t;  // the enumeration position of a cex
...
struct concat_t : public sygus_op
{
  uint64_t width1;
  uint64_t width2;
  concat_t(uint64_t w1, uint64_t w2) : width1(w1), width2(w2) {}
...
struct rotate_t : public sygus_op
{
  smt::PrimOp op;
  uint64_t param;
  rotate_t(smt::PrimOp _op, uint64_t _param) : op(_op), param(_param) {}
  virtual smt::Op to_smt_op() const override { return smt::Op(op, param); }
};
struct extend_t : public sygus_op
{
  smt::PrimOp op;
  uint64_t extw, input_width;
  extend_t(smt::PrimOp _op, uint64_t _extw, uint64_t _iw)
      : op(_op), extw(_extw), input_width(_iw)
  {
  }
  virtual smt::Op to_smt_op() const override { return smt::Op(op, extw); }
};
...
struct BvConstructs
{
  ...
  std::unordered_set<smt::PrimOp>
      op_unary;  // unary operators: (_ bv x) -> (_ bv x)
  std::unordered_set<smt::PrimOp>
      op_binary;  // binary operators: (_ bv x) x (_ bv x) -> (_ bv x)
  std::unordered_set<smt::PrimOp>
      op_comp;  // comparators: (_ bv x) x (_ bv x) -> bool
...
class SyntaxStructure
{
 public:
  ...
  const SyntaxT & GetSyntaxConstruct() const { return syntax_; }
  ...
  void CutVars(const std::unordered_set<std::string> & keep_vars_name,
