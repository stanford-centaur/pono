...
{
 public:
  syntax_analysis::SyntaxStructure & GetSyntaxConstruct() { return constructs; }
...
class TermExtractor : public Walker
{
 public:
  ...
  // ----------- CONSTRUCTOR --------------- //
  // if level > 0, then we will count level, otherwise, we don't care about the
  // levels
  TermExtractor(
      ...
      : related_vars_(varset),
        collect_constants_(collect_constants),
        level_(level),
        width_to_terms_(width_to_term_table),
        all_terms_(all_terms_set)
  ...
  // }
  // const std::unordered_set<smt::Term> & GetAllTerms() const
  // {
  //   return all_terms_;
  // }
 ...
 protected:
  ...
  const std::unordered_set<smt::Term> &
      related_vars_;  // we will also bring in unrelated vars
...
class ParentExtract : public Walker
{
 public:
  ...
  bool RegisterNewParentRelation(const smt::Term & child,
...
class TermLearner
{
 public:
  TermLearner(to_next_t to_next_func,
              ...
              smt::SmtSolver & btor,
              ParentExtract & parent_extractor)
  ...
  unsigned learn_terms_from_cex(IC3FormulaModel * pre,
                                IC3FormulaModel * post,
                                const smt::Term & trans,
                                /*OUTPUT*/ PerVarsetInfo & varset_info);

  unsigned learn_terms_from_cex_same_frame(
      IC3FormulaModel * pre,
      IC3FormulaModel * post,
      const smt::Term & trans,
      /*OUTPUT*/ PerVarsetInfo & varset_info);

  unsigned vars_extract_bit_level(IC3FormulaModel * post,
                                  /*OUTPUT*/ PerVarsetInfo & varset_info);
 ...
 protected:
  ...
  unsigned same_val_replace_ast_same_frame(
      /*INOUT*/ PerVarsetInfo & varset_info);
  unsigned replace_hierachically(const smt::Term & orig,
                                 const smt::Term & repl,
                                 /*INOUT*/ PerVarsetInfo & varset_info);

  unsigned replace_hierachically_w_parent(const smt::Term & orig,
                                          const smt::Term & repl,
