    ...
    : super(p, ts, solver, opt)
{
  ...
  sel_neg_bad_state_terms_ =
      solver_->make_symbol("sel_neg_bad_state_terms_", boolsort);
  not_sel_neg_bad_state_terms_ =
      solver_->make_term(Not, sel_neg_bad_state_terms_);

  // add selector term to toggle simple path constraints
  sel_simple_path_terms_ =
      solver_->make_symbol("sel_simple_path_terms_", boolsort);
  not_sel_simple_path_terms_ = solver_->make_term(Not, sel_simple_path_terms_);

  // Note on selector literals: as a potential optimization, we could enforce
  // the values of selector literals permanently by adding unit clauses rather
  // than setting the literal via assumptions. E.g., in the default
  // configuration of k-induction, most constraints like negated bad state terms
  // and simple path constraints are added permanently and are never removed. So
  // the value of their respective selective literal is never flipped and can be
  // set permanently. E.g., this can be done as follows:
  //
...
ProverResult KInduction::check_until(int k)
{
  ...
  if (bound_step_ != 1 && options_.kind_eager_simple_path_check_)
    throw PonoException(
        "Must not combine '--kind-bound-step <n>' with 'n > 1' "
        "and '--kind-eager-simple-path-check'");

  // NOTE: When unrolling in intervals of length > 1 ('bound_step_ > 1'), we
  // must add a disjunctive bad state property covering all bounds in regular
  // base case checks (not yet implemented), unless we skip regular base checks
  // by option
  // '--kind-one-time-base-check'.
  // With '--kind-one-time-base-check' and after an inductive case check was
  // unsatisfiable, a disjunctive bad state property will be checked in a single
  // base case check (function 'final_base_case_check(..)') to make sure no
  // counterexamples are missed; Hence currently option '--kind-bound-step <n>'
  // with 'n > 1' must be combined with option '--kind-one-time-base-check'.
  if (bound_step_ != 1 && !options_.kind_one_time_base_check_)
    throw PonoException(
        "Must combine '--kind-bound-step <n>' with 'n > 1' "
  ...
  for (int i = reached_k_ + 1; i <= k; i += bound_step_) {
    ...
    if (!options_.kind_no_simple_path_check_) {
      ...
      if (ts_.statevars().size()
          && ((!options_.kind_eager_simple_path_check_
               && check_simple_path_lazy(i))
              || (options_.kind_eager_simple_path_check_
                  && check_simple_path_eager(i)))) {
        if (options_.kind_one_time_base_check_) {
    ...
    if (i >= 1 && !options_.kind_no_ind_check_init_states_) {
      ...
      kind_log_msg(
          1, "", "checking inductive step (initial states) at bound: {}", i);
    ...
    // open new frame; this is to be able to remove bad state predicate added
    // next
    solver_->push();
