...
if [ ! -d "$DEPS/mathsat" ]; then
    ...
    if [[ "$OSTYPE" == linux* ]]; then
        wget -O mathsat.tar.gz https://mathsat.fbk.eu/download.php?file=mathsat-5.6.8-linux-x86_64.tar.gz
    elif [[ "$OSTYPE" == darwin* ]]; then
        wget -O mathsat.tar.gz https://mathsat.fbk.eu/download.php?file=mathsat-5.6.8-osx.tar.gz
    elif [[ "$OSTYPE" == msys* ]]; then
        wget -O mathsat.tar.gz https://mathsat.fbk.eu/download.php?file=mathsat-5.6.8-win64-msvc.zip
