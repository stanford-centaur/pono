...
test.out: test.cpp btor2tools/build/libbtor2parser.a frontends/btor2_encoder.cpp core/rts.cpp
	$(CXX) $(DEBUG) -std=c++17 -I./utils/ -I./core -Ibtor2tools/src/ -I. -I$(SMTSWITCHDIR)/include -L$(SMTSWITCHDIR)/lib -Wl,-rpath=$(SMTSWITCHDIR)/lib -o test.out test.cpp bmc.cpp kinduction.cpp unroller.cpp btor2tools/build/libbtor2parser.a core/rts.cpp core/fts.cpp core/prop.cpp frontends/btor2_encoder.cpp -lsmt-switch -lsmt-switch-btor -lgmpxx -lgmp
