...
if [ ! -d "$DEPS/mathsat" ]; then
    ...
    if [[ "$OSTYPE" == linux* ]]; then
        wget -O mathsat.tar.gz https://mathsat.fbk.eu/release/mathsat-5.6.3-linux-x86_64.tar.gz
    elif [[ "$OSTYPE" == darwin* ]]; then
        wget -O mathsat.tar.gz https://mathsat.fbk.eu/release/mathsat-5.6.3-darwin-libcxx-x86_64.tar.gz
    elif [[ "$OSTYPE" == msys* ]]; then
        wget -O mathsat.tar.gz https://mathsat.fbk.eu/release/mathsat-5.6.3-win64-msvc.zip
