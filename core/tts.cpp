...
bool TimedTransitionSystem::is_linear(const smt::Term & term,
                                      bool & has_clocks) const
{
  smt::Op op = term->get_op();
  has_clocks = false;

  if (clock_vars_.find(term) != clock_vars_.end()) has_clocks = true;
  if (op.is_null()) {
    return true;
  }

  TermVec children(term->begin(), term->end());
  bool has_clocks1, has_clocks2;
  bool is_linear1, is_linear2;
  switch (op.prim_op) {
    case smt::Plus:
    case smt::Minus:
      is_linear1 = is_linear(children[0], has_clocks1);
      is_linear2 = is_linear(children[1], has_clocks2);
      has_clocks = has_clocks1 | has_clocks2;
      return is_linear1 && is_linear2;
    case smt::Mult:
      is_linear1 = is_linear(children[0], has_clocks1);
      is_linear2 = is_linear(children[1], has_clocks2);
      has_clocks = has_clocks1 | has_clocks2;
      return is_linear1 && is_linear2 && !(has_clocks1 && has_clocks2);
    case smt::Div:
      is_linear1 = is_linear(children[0], has_clocks1);
      is_linear2 = is_linear(children[1], has_clocks2);
      has_clocks = has_clocks1 | has_clocks2;
      return is_linear1 && is_linear2 && !(has_clocks2);
    case smt::Negate:
    case smt::To_Int:
    case smt::To_Real:
      is_linear1 = is_linear(children[0], has_clocks1);
      has_clocks = has_clocks1;
      return is_linear1;
    default: return false;
  }
}

bool TimedTransitionSystem::is_valid_atomic_invariant(
    const smt::Term & term) const
{
  smt::Op op = term->get_op();
  if (!this->contains_clocks(term)) {
    return true;
  }
  ...
  switch (op.prim_op) {
    ...
    case smt::Equal:
      bool has_clocks1, has_clocks2;
      return is_linear(children[0], has_clocks1)
             && is_linear(children[1], has_clocks2);
    case smt::Not: return is_valid_atomic_invariant(children[0]);
    default: return false;
  }
}

bool TimedTransitionSystem::is_valid_invariant(const smt::Term & term) const
{
  smt::Op op = term->get_op();
  if (op.is_null()) return true;
  smt::Term conjunction = term;
  TermVec children(term->begin(), term->end());
  if (op.prim_op == Implies) {
    if (contains_clocks(children[0])) return false;
    conjunction = children[1];
  } else if (op.prim_op == Or) {
    // Or is OK if it encodes an implication as above
    // ~children[0] => children[1]
    if (!contains_clocks(children[0])) {
      conjunction = children[1];
    } else if (!contains_clocks(children[1])) {
      // ~children[1] => children[0]
      conjunction = children[0];
    } else
      return false;
  }
  smt::TermVec conjuncts;
  conjunctive_partition(conjunction, conjuncts);
  for (auto conjunct : conjuncts) {
    if (!this->is_valid_atomic_invariant(conjunct)) return false;
  }
  return true;
}
...
void TimedTransitionSystem::encode_compact_delays()
{
  ...
  // C >= 0 /\ C' >= 0
  for (auto c : clock_vars_) {
    add_invar(solver_->make_term(Ge, c, zero));
  }
