...
class TimedTransitionSystem : public RelationalTransitionSystem
{
 public:
  ...
  void add_invar(const smt::Term & inv)
  {
    ...
    if (contains_clocks(inv)) {
      if (!is_valid_invariant(inv)) {
        throw PonoException("The following is not a valid clock invariant: "
                            + inv->to_string());
      }
  ...
  /**
   * Add an urgency constraint: the global urgency constraint is the
   * disjunction of all added constraints, which all describe urgent states.
 ...
 protected:
  ...
  /**
   * Given trans(C, X, I, C', X'), add a fresh delay variable, and redefine
   * the trans relation as
   *
   * T(C, X, I, C',X') =
   *    C >= 0
   * /\ delta >= 0
  ...
  /**
   * Whether term describes a linear expression as a function of clocks
   */
  bool is_linear(const smt::Term & term, bool & has_clocks) const;
  /**
   * Whether term encodes the comparison (<, <=, >, >=, =) of two linear terms
   * (linear as a function of clocks)
   * @see is_linear
   */
  bool is_valid_atomic_invariant(const smt::Term & term) const;
  /**
   * Whether term has one of the following forms:
   * 1. A conjunction of valid atomic invariants
   * 2. An implication of the form A -> B where A has no clocks, and B
   * satisfies 1.
   * @see is_valid_atomic_invariant
   */
  bool is_valid_invariant(const smt::Term & term) const;
